...
public interface TotalGoalRepository extends JpaRepository<TotalGoal, Long> {
	...
	@Query(value = "SELECT DATE_FORMAT(DATE_SUB(goal_date, INTERVAL 1 MONTH), '%Y-%m') AS before_month " +
		"FROM total_goal t " +
		"WHERE t.member_id = ?1 AND DATE_FORMAT(t.goal_date, '%Y-%m') = DATE_FORMAT(?2, '%Y-%m')", nativeQuery = true)
	String findBeforeMonth(UUID memberId, Timestamp curDate);

	@Query(value = "SELECT DATE_FORMAT(DATE_ADD(goal_date, INTERVAL 1 MONTH), '%Y-%m') AS before_month " +
		"FROM total_goal t " +
		"WHERE t.member_id = ?1 AND DATE_FORMAT(t.goal_date, '%Y-%m') = DATE_FORMAT(?2, '%Y-%m')", nativeQuery = true)
